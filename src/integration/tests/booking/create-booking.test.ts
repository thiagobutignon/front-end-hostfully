...
describe('CreateBookingUsecase Integration Test', () => {
  ...
  test('should not add two bookings in the same day', async () => {
    ...
    expect(cacheBookingSingleton.getAll()).toHaveLength(1)

    await expect(sut.perform(bookingData2)).rejects.toThrow(
  ...
  const testCases = [
    {
      ...
      numberOfGuests: 1,
      maxGuests: 10,
      errorMessage: new DateError().message
    },
    {
      propertyId: '1',
      startDate: 1,
      ...
      numberOfGuests: 12,
      maxGuests: 10,
      errorMessage: new InvalidCredentialError().message
    }
  ]

  test.each(testCases)(
    'should fail to create a booking with %p',
    async ({
      propertyId,
      startDate,
      endDate,
      ...
      await expect(sut.perform(bookingData)).rejects.toThrow(
